...
class Notification:
    ...
    def send_appointment_notification(self, vaccines, department):
        ...
        if 'ok' in errmsg:
            self.logging.info(f"{self.city}群发送{department}{vaccines}疫苗可预约通知成功")
        else:
            self.logging.error(f"{self.city}群发送{department}{vaccines}疫苗可预约通知失败，请检查~\n"
                               f"错误信息：{res}")

    def send_error_notification(self, vaccines, message):
        """发送服务异常通知"""
        url = self.notification_urls["test"]
        data = {"msgtype": "text",
                "text": {
                    "content":
                        f"{self.city}{vaccines}服务异常提醒\n"
                        "\n"
                        f"{message}\n"
                        f"时间:  {self.send_time}\n"
                },
        ...
        errmsg = jsonpath(res, "$.errmsg")
        if 'ok' in errmsg:
            self.logging.info(f"{self.city}群发送{vaccines}服务异常提醒成功")
        else:
            self.logging.error(f"{self.city}群发送{vaccines}服务异常提醒失败，请检查~\n"
                               f"错误信息：{res}")
